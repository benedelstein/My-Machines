...
- [x] S1.4 Teardown revokes Git capabilities and deletes the DO webhook token before slow external cleanup, then deletes the internal connector. It does not edit class-B policies. `deleteForTeardown` marks `pending_revocation`, deletes + verifies, then removes the row.

## S2. Git cutover

- [x] S2.1 Interim Git cutover for the gateway `Accept` bug: the session connector exposes only an exact JSON capability-mint endpoint. The DO validates the connector-injected webhook token and issues a five-minute opaque capability from its SQLite secrets store. A repo-local credential helper obtains it, then fetch/push go directly to the Worker Git proxy with HTTP Basic. Capability mode rejects the legacy bearer and webhook token and preserves per-request repository access. Pre-connector sessions remain in explicit `legacy_secret` mode.
...
- [ ] S2.3 Initial clone remains on direct GitHub with its short-lived contents-read-only token. New sessions no longer create `gitProxySecret`; both later fetch and push use the direct Worker proxy with rotating capabilities. Post-clone read latency still needs live measurement.
- [ ] S2.4 Return post-clone Git to the per-session connector after Fly fixes gateway content negotiation: first require `test:live:gateway-accept` to pass Git's `application/x-git-*` cases end to end; then allowlist `/git-proxy/:sessionId/*`, point both remotes to the gateway, accept only the connector-injected session token for connector-mode Git, and stop installing the helper or issuing capabilities for new sessions. Preserve capability mode only until existing sessions migrate or drain, then remove the mint endpoint and capability state.
...
- [ ] S3.5 Webhook, Git capability mint/refresh, and provider traffic use direct gateway URLs; while the Fly `Accept` bug remains, Git smart-HTTP data uses the direct Worker URL, then S2.4 moves it back to the gateway. None enters the class-B proxy. Install the data plane only for environments with class-B credentials; lifecycle (start at provision, tear down all of it at end).
...
- [ ] 7.4 Identity tests: off-Sprite webhook/environment credential replay and Git capability mint/refresh are rejected; a still-valid extracted Git capability remains replayable until expiry or DO revocation; another Sprite is denied.

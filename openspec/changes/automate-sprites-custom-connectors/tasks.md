...
- [ ] 1.6 Endpoint pinning: the provisioner accepts `allowedEndpoints`, verifies it on read-back, and requires it for `session:` connectors (2026-07-26). Wire format verified against the live Sprites API the same day: `allowed_endpoints: ["/headers"]` accepted on PATCH and returned verbatim on GET (disposable live test, 14.2s total). Remaining: verify the gateway actually blocks off-pin paths from a labelled Sprite once labeling lands (needs an in-VM test; pairs with 1.5).

...
- [ ] S2.3 Initial clone remains on direct GitHub with its short-lived contents-read-only token. New sessions no longer create `gitProxySecret`; both later fetch and push use the direct Worker proxy with rotating ephemeral tokens. Post-clone read latency still needs live measurement.
- [ ] S2.4 Return post-clone Git to the per-session connector after Fly fixes gateway content negotiation: first require `test:live:gateway-accept` to pass Git's `application/x-git-*` cases end to end; then allowlist `/git-proxy/:sessionId/*`, point both remotes to the gateway, accept only the connector-injected session token for connector-mode Git, and stop installing the helper or issuing ephemeral tokens for new sessions. Preserve ephemeral-token mode only until existing sessions migrate or drain, then remove the mint endpoint and token state.
...
- [ ] S3.5 Webhook, ephemeral Git token mint/refresh, and provider traffic use direct gateway URLs; while the Fly `Accept` bug remains, Git smart-HTTP data uses the direct Worker URL, then S2.4 moves it back to the gateway. None enters the class-B proxy. Install the data plane only for environments with class-B credentials; lifecycle (start at provision, tear down all of it at end).
...
- [ ] S4.2 Extend class-A routing with `/internal/session/:sessionId/inference/{claude|codex}/...`; validate the existing connector-injected session credential through the DO, resolve the authenticated session user, and use the existing encrypted provider record. Add no provider connector, owner label, or connector table. Validate-then-stream: the DO hop is one small token-validation RPC returning the session user; the provider stream is handled entirely Worker-side and never passes through the DO (see the inference constraint in design.md).
...
- [ ] 7.4 Identity tests: off-Sprite webhook/environment credential replay and ephemeral Git token mint/refresh are rejected; a still-valid extracted ephemeral Git token remains replayable until expiry or DO revocation; another Sprite is denied.

...
- Keep writes simple and tied to relevant state mutations and view disappearance.
...
For these fields, the view reads `clientState` directly. The summary seeds its initial overlapping values when the view model is created, and the cached snapshot replaces those values while reconnecting. The summary remains the source for title, which is not part of client state. The active-turn ID is reduced to a boolean because only the working presentation needs restoration.
...
The current summary model seeds title and initial client-state values. The cached snapshot then restores the canonical session fields. A live client-state frame replaces the curated values, and a sync response replaces the responding value from its active-turn state.

### D4. Writes follow relevant state mutations

The view model derives and saves a cached snapshot after live-state, response-state, setup, model, branch, or pull-request mutations. It also saves the latest value before `unbind()` resets response state.

This avoids a timer, diff tracking, or a separate synchronization state machine.
...
3. Add cache-first hydration and mutation-triggered snapshot writes to the view model.

...
- **Connector provisioning** - `@repo/sprites-client` owns the Sprites REST wire format. The API-server connector integration owns validation plus create/verify/delete orchestration and uses the existing `SPRITES_API_KEY`; there is no separate provisioning service. The `session.connector-resource` runtime migration maintains one label-scoped connector per session from authoritative direct-ID reads, including sessions created before connectors existed. While Fly's gateway rejects Git smart-HTTP Accept headers, the connector authenticates a control-plane mint for five-minute ephemeral Git tokens and Git sends those tokens directly to the Worker proxy. See `docs/sprite-connectors.md`.

...
- **Connector provisioning** - `@repo/sprites-client` owns the Sprites REST wire format. The API-server shared connector integration owns validation plus create/verify/delete orchestration and uses the API server's existing `SPRITES_API_KEY`; there is no separate provisioning service. Each session mints one label-scoped connector during provisioning so the Sprite never holds the webhook credential. While Fly's gateway rejects Git smart-HTTP Accept headers, the connector authenticates a control-plane mint for five-minute Git capabilities and Git sends those capabilities directly to the Worker proxy. Sessions provisioned before connectors existed keep their legacy Sprite-held paths until they age out. See `docs/sprite-connectors.md`.

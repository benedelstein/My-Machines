...
- **Connector provisioning** - `@repo/sprites-client` owns the Sprites REST wire format. The API-server shared connector integration owns validation plus create/verify/delete orchestration and uses the API server's existing `SPRITES_API_KEY`; there is no separate provisioning service. See `docs/sprite-connectors.md`.

...
- **Connector provisioning** - `@repo/sprites-client` owns the Sprites REST wire format. The API-server connector integration owns validation plus create/verify/delete orchestration and uses the existing `SPRITES_API_KEY`; there is no separate provisioning service. The `sprite.session-labels` runtime migration owns the Sprite's exact session/environment label set, and the later `session.connector-resource` migration maintains one connector whose policy requires the session label. Both reconcile legacy sessions from authoritative provider reads. Fly's gateway rejects Git smart-HTTP media types and buffers provider SSE, so the connector authenticates only narrow control-plane mints for five-minute Git and planned inference capabilities. Git data goes directly to the Worker proxy; planned provider streams go directly through a shared Node provider proxy after a Worker/DO credential-control check. See `docs/sprite-connectors.md`.

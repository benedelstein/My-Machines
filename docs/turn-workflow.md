...
`agentProcessRunId` is generated by `SpriteAgentProcessManager` for each
fresh vm-agent spawn and is used by `AgentTurnCoordinator` to ignore stale
`process_exit` events from earlier processes.

Only one user turn may be active per session. A second `chat.message` while
`activeUserMessageId` or the synced client state's `pendingUserMessage` is set
is rejected with `CHAT_MESSAGE_FAILED`.
...
from the setup session. `SpriteAgentProcessManager` also records the generated
process run id in `server_state.agentProcessRunId`. New sessions receive the
connector gateway in `DO_WEBHOOK_URL` with `DO_WEBHOOK_AUTH=gateway` and no
VM-held bearer token; the gateway injects the session credential after
verifying the Sprite label. Pre-connector sessions retain the explicit legacy
fallback: the Worker URL plus `DO_WEBHOOK_AUTH=bearer` and `DO_WEBHOOK_TOKEN`.
...
Internal routes in `services/api-server/src/modules/session-agent/routes/internal.routes.ts`
extract an `Authorization: Bearer ...` credential and forward it to the owning
Durable Object. For connector-backed sessions, that bearer value is injected by
the Sprites gateway from the session's connector credential. For legacy
pre-connector sessions, the vm-agent sends the Sprite-held `DO_WEBHOOK_TOKEN`.
`SessionAgentDO.isWebhookTokenValid(...)` compares the value with
`webhook_token` in `SecretRepository`.
...
`process_exit` carries the vm-agent process run id. `AgentTurnCoordinator`
commits any active partial message as aborted only when the event's
`processRunId` matches `server_state.agentProcessRunId`; stale process exits are
logged and ignored.
